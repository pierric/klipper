...
static double deltesian_stepper_calc_position(struct stepper_kinematics *sk,
                                              struct move *m,
                                              double move_time) {
  struct deltesian_stepper *ds = container_of(sk, struct deltesian_stepper, sk);
  struct coord c = move_get_coord(m, move_time);
  double dx = c.x - ds->arm_x;
  return sqrt(ds->arm2 - dx * dx) + c.z;
}

struct stepper_kinematics *__visible deltesian_stepper_alloc(double arm2,
                                                             double arm_x) {
