...
inline double move_get_distance(struct move *m, double move_time) {
  return (m->start_v + m->half_accel * move_time) * move_time;
}

// Return the XYZ coordinates given a time in a move
inline struct coord move_get_coord(struct move *m, double move_time) {
  double move_dist = move_get_distance(m, move_time);
  return (struct coord){.x = m->start_pos.x + m->axes_r.x * move_dist,
...
struct trapq *__visible trapq_alloc(void) {
...
void __visible trapq_free(struct trapq *tq) {
...
void __visible trapq_append(struct trapq *tq, double print_time, double accel_t,
                            double cruise_t, double decel_t, double start_pos_x,
                            double start_pos_y, double start_pos_z,
                            double start_pos_a, double start_pos_b,
                            double start_pos_c, double axes_r_x,
                            double axes_r_y, double axes_r_z, double axes_r_a,
                            double axes_r_b, double axes_r_c, double start_v,
  ...
  struct coord start_pos = {.x = start_pos_x,
                            ...
                            .b = start_pos_b,
                            .c = start_pos_c};
  struct coord axes_r = {.x = axes_r_x,
                         .y = axes_r_y,
                         .z = axes_r_z,
                         .a = axes_r_a,
                         .b = axes_r_b,
...
void __visible trapq_finalize_moves(struct trapq *tq, double print_time,
...
int __visible trapq_extract_old(struct trapq *tq, struct pull_move *p, int max,
