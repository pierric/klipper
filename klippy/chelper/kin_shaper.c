...
static void shift_pulses(struct shaper_pulses *sp) {
  int i;
  double ts = 0.;
...
static int init_shaper(int n, double a[], double t[],
...
static inline double get_axis_position(struct move *m, int axis,
                                       double move_time) {
...
static inline double get_axis_position_across_moves(struct move *m, int axis,
                                                    double time) {
  while (likely(time < 0.)) {
...
static inline double calc_position(struct move *m, int axis, double move_time,
                                   struct shaper_pulses *sp) {
  double res = 0.;
...
static double shaper_x_calc_position(struct stepper_kinematics *sk,
...
static double shaper_y_calc_position(struct stepper_kinematics *sk,
                                     struct move *m, double move_time) {
  struct input_shaper *is = container_of(sk, struct input_shaper, sk);
  if (!is->sy.num_pulses)
    return is->orig_sk->calc_position_cb(is->orig_sk, m, move_time);
...
static double shaper_xy_calc_position(struct stepper_kinematics *sk,
                                      struct move *m, double move_time) {
  struct input_shaper *is = container_of(sk, struct input_shaper, sk);
...
int __visible input_shaper_set_sk(struct stepper_kinematics *sk,
                                  struct stepper_kinematics *orig_sk) {
  struct input_shaper *is = container_of(sk, struct input_shaper, sk);
...
static void shaper_note_generation_time(struct input_shaper *is) {
  ...
  if ((is->sk.active_flags & AF_Y) && is->sy.num_pulses) {
    pre_active = is->sy.pulses[is->sy.num_pulses - 1].t > pre_active
                     ? is->sy.pulses[is->sy.num_pulses - 1].t
                     : pre_active;
    post_active =
        -is->sy.pulses[0].t > post_active ? -is->sy.pulses[0].t : post_active;
  }
  is->sk.gen_steps_pre_active = pre_active;
  is->sk.gen_steps_post_active = post_active;
}

int __visible input_shaper_set_shaper_params(struct stepper_kinematics *sk,
...
input_shaper_get_step_generation_window(struct stepper_kinematics *sk) {
  ...
  return is->sk.gen_steps_pre_active > is->sk.gen_steps_post_active
             ? is->sk.gen_steps_pre_active
             : is->sk.gen_steps_post_active;
}

struct stepper_kinematics *__visible input_shaper_alloc(void) {
