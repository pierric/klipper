...
static double polar_stepper_radius_calc_position(struct stepper_kinematics *sk,
                                                 struct move *m,
                                                 double move_time) {
...
static double polar_stepper_angle_calc_position(struct stepper_kinematics *sk,
                                                struct move *m,
                                                double move_time) {
  struct coord c = move_get_coord(m, move_time);
  // XXX - handle x==y==0
...
static void polar_stepper_angle_post_fixup(struct stepper_kinematics *sk) {
...
struct stepper_kinematics *__visible polar_stepper_alloc(char type) {
