...
static int32_t itersolve_gen_steps_range(struct stepper_kinematics *sk,
                                         struct move *m, double abs_start,
                                         double abs_end) {
  ...
  double last_time = start, low_time = start,
         high_time = start + SEEK_TIME_RESET;
  if (high_time > end)
    high_time = end;
  for (;;) {
    ...
    if (ret)
      return ret;
    target =
        sdir ? target + half_step + half_step : target - half_step - half_step;
...
static inline int check_active(struct stepper_kinematics *sk, struct move *m) {
  ...
  return (
      (af & AF_X && m->axes_r.x != 0.) || (af & AF_Y && m->axes_r.y != 0.) ||
      (af & AF_Z && m->axes_r.z != 0.) || (af & AF_A && m->axes_r.a != 0.) ||
      (af & AF_B && m->axes_r.b != 0.) || (af & AF_C && m->axes_r.c != 0.));
}

// Generate step times for a range of moves on the trapq
int32_t __visible itersolve_generate_steps(struct stepper_kinematics *sk,
                                           double flush_time) {
  ...
  for (;;) {
    ...
    if (check_active(sk, m)) {
      if (skip_count && sk->gen_steps_pre_active) {
        ...
        do {
          int32_t ret =
              itersolve_gen_steps_range(sk, pm, abs_start, flush_time);
          if (ret)
            return ret;
      ...
      int32_t ret =
          itersolve_gen_steps_range(sk, m, last_flush_time, flush_time);
      if (ret)
        return ret;
      ...
      if (move_start < force_steps_time) {
        ...
        if (abs_end > flush_time)
          abs_end = flush_time;
        int32_t ret =
            itersolve_gen_steps_range(sk, m, last_flush_time, abs_end);
        if (ret)
          return ret;
        skip_count = 1;
      } else {
...
double __visible itersolve_check_active(struct stepper_kinematics *sk,
                                        double flush_time) {
  if (!sk->tq)
    return 0.;
  trapq_check_sentinels(sk->tq);
...
int32_t __visible itersolve_is_active_axis(struct stepper_kinematics *sk,
                                           char axis) {
...
void __visible itersolve_set_trapq(struct stepper_kinematics *sk,
                                   struct trapq *tq) {
  sk->tq = tq;
}

void __visible itersolve_set_stepcompress(struct stepper_kinematics *sk,
...
double __visible itersolve_calc_position_from_coord(
    struct stepper_kinematics *sk, double x, double y, double z, double a,
    double b, double c) {
  struct move m;
  memset(&m, 0, sizeof(m));
  m.start_pos.x = x;
...
void __visible itersolve_set_position(struct stepper_kinematics *sk, double x,
